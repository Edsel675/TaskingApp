...
import { Pencil, Type, Trash, ChevronRight, Folder, Save, X, Edit, FileText } from 'lucide-react';
...
const NoteTaskApp = () => {
  ...
  const [drawings, setDrawings] = useState<{id: string, strokes: string[], x: number, y: number, viewBox: string}[]>([]);
  const [textBoxes, setTextBoxes] = useState<{id: string, content: string, x: number, y: number}[]>([]);
  ...
  const updateTextBox = async (id: string, newContent: string) => {
    setTextBoxes(prev => prev.map(box => box.id === id ? {...box, content: newContent} : box));
    try {
  ...
  const moveTextBox = async (id: string, dx: number, dy: number) => {
    const updatedTextBoxes = textBoxes.map(box => 
      box.id === id 
        ? { ...box, 
            x: Math.max(0, Math.min(box.x + dx, workspaceRef.current!.clientWidth - 200)), 
            y: Math.max(0, Math.min(box.y + dy, workspaceRef.current!.clientHeight - 100)) 
          }
        : box
    );
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (canvas) {
      ...
      setCurrentStrokes(prev => [...prev, [{x: e.clientX - rect.left, y: e.clientY - rect.top}]]);
    }
  };
  ...
  const saveDrawing = async () => {
    if (currentStrokes.length > 0 && workspaceRef.current) {
      ...
      const scale = Math.min(DRAWING_SIZE / drawingWidth, DRAWING_SIZE / drawingHeight);
      ...
      const newDrawing = {
        strokes: scaledStrokes.map(stroke => stroke.map(p => `${p.x},${p.y}`).join(' ')),
      ...
      try {
        if (editingDrawing) {
          ...
          setDrawings(prevDrawings => prevDrawings.map(d => d.id === editingDrawing ? { ...newDrawing, id: editingDrawing } : d));
        } else {
          const docRef = await addDoc(collection(db, "drawings"), newDrawing);
          setDrawings(prevDrawings => [...prevDrawings, { ...newDrawing, id: docRef.id }]);
        }
        setCurrentStrokes([]);
        setEditingDrawing(null);
        setMode('view');
  ...
  const moveDrawing = async (id: string, dx: number, dy: number) => {
    const updatedDrawings = drawings.map(drawing => 
      drawing.id === id 
        ? { ...drawing, 
            x: Math.max(0, Math.min(drawing.x + dx, workspaceRef.current!.clientWidth - DRAWING_SIZE)), 
            y: Math.max(0, Math.min(drawing.y + dy, workspaceRef.current!.clientHeight - DRAWING_SIZE)) 
          }
        : drawing
    );
  ...
  const editDrawing = (id: string) => {
    ...
    if (drawingToEdit) {
      ...
      setMode('draw');
      setCurrentStrokes(drawingToEdit.strokes.map(stroke => 
  ...
  return (
    <div className="flex h-screen bg-background text-foreground font-sans">
      ...
      <div className="w-20 bg-secondary p-4 flex flex-col items-center space-y-6">
        ...
        <FileText size={24} onClick={handleNotepadClick} className="cursor-pointer" />
        <Pencil size={24} onClick={handlePencilClick} className="cursor-pointer" />
        <Trash size={24} />
      ...
      {/* Workspace */}
      <div ref={workspaceRef} className="flex-grow bg-muted p-6 relative overflow-hidden">
        <h2 className="text-sm font-bold absolute top-4 left-1/2 transform -translate-x-1/2">workspace</h2>
        {mode === 'write' && (
              ...
              >
                <svg
                  ...
                  onMouseDown={(e) => {
                    ...
                    const moveHandler = (e: MouseEvent) => {
                      moveDrawing(drawing.id, e.clientX - startX, e.clientY - startY);
                    };
                    const upHandler = () => {
                ...
                >
                  {drawing.strokes.map((stroke, index) => (
                    <path key={index} d={`M ${stroke}`} fill="none" stroke="black" strokeWidth="2" />
                  ))}
                </svg>
                <div className="absolute bottom-0 left-0 right-0 flex justify-center p-1 bg-opacity-50 bg-white">
                  <button onClick={() => editDrawing(drawing.id)} className="mr-2">
                    <Edit size={16} />
                  </button>
      ...
      <div className="w-64 bg-secondary p-4 overflow-y-auto">
        ...
        <div className="space-y-4">
          {tasks.length > 0 ? (
            tasks.map((task) => (
              <div key={task.id} className="bg-card p-2 rounded flex justify-between items-start">
                <div>
